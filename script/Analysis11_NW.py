...
def compileouting(climbout,fithash,condition,compileout):
  infile = open(climbout,'r')
  localmaxs = {}
  for line in infile.xreadlines():
    ...
    path     = line[2]
    localmax = line[3]
    if localmaxs.has_key(localmax): localmaxs[localmax] += 1
    else: localmaxs[localmax] = 1
  infile.close()
  outfile = open(compileout,'w')
  outfile.write("\t".join(['mut','basin','I20fit'])+"\n")
  [outfile.write("\t".join(map(str,[mut,localmaxs[mut],fithash[mut][condition]]))+"\n") for mut in localmaxs.keys()]
  outfile.close()
...
def main():
  ...
  print 'Among these, # of variants as a local max: %s' % localmax_count
