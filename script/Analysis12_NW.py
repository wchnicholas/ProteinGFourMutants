...
def labelnode(var,fit):
  if fit == 'NA': return 'red'
  scale = int(round((8-fit)*8.75+30))
  if fit >= 8: return 'grey30'
  else: return 'grey'+str(scale)
 
def drawgraph(nodes,outfile,scalefile,fithash,condition):
  normscale = 50000
  scaling   = 4
...
def main():
  graphfile    = 'xdot/LocalMax.dot'
  graphscale   = 'xdot/LocalScale.dot'
  WT           = 'VDGV'
  condition    = 'I20fit'
  fcutoff      = float(1)
  ...
  drawgraph(localmaxs,graphfile,graphscale,localmaxhash,condition)
