...
def monoincr(path,fithash,condition,mut,WT):
  for n in range(1,len(path)):
    ...
    if n==1 and float(fithash[stepCurr][condition]) < 0.01: return 0
  return 1
...
def pathwayparam(fithash,muts,WT,condition,outfile,HDdist,fixtype):
  ...
  for mut in muts:
    ...
    if mut == 'YGCM' or mut == 'TGYE' or mut == 'FPKQ': continue
    if HD == HDdist and mutfit > 0.01 and mutfit < 1:
      countmut += 1
      ...
      nodeQ = 0
      pathQ = 0
      for localmax in localmaxs: localmonopaths = monolocalmaxing(localmax,mut,fithash,condition,localmonopaths,G)
      if len(localmonopaths[WT]) == 0: continue
      [localmonomaxs.append(localmonomax) for localmonomax in localmonopaths.keys() if len(localmonopaths[localmonomax]) != 0]
      if len(localmonomaxs) != 1 or hamming(localmonomaxs[0],mut) != 4: print 'Something is wrong upstream'; continue
      for step in G.nodes(): stepprobhash = stephashing(stepprobhash,step,WT,mut,fithash,condition,fixtype)
      ...
      #print 'Scale for path normalization: %s for %s' % (pathprobnormalscale, mut)
      if str(pathprobnormalscale) != '1.0': print mut, str(pathprobnormalscale); problematicnodes.append(mut); continue
  ...
  print 'For HD = %d, there are %d low fitness variants with a monotonic paths to WT' % (HDdist, countmutwithmonopath)
  print 'There are %d problematic subgraphs to WT' % len(problematicnodes)
  print "These are: ", problematicnodes
      #outfile.write(mut+"\t"+str(giniindex)+"\t"+"\t".join(map(str,accprob_paths))+"\n")
      
def main():
  WT          = 'VDGV'
  fitfile     = 'result/Mutfit'
  outfile     = 'analysis/PathwayParamResultToWT'
